...
# merges accepted matches with cluster-level conflict safeguards.
#
...
OUTPUT_DIRECTORY = Path("identity_resolution") / "probabilistic"
...
AUTOMATIC_MATCH_THRESHOLD = 0.85
...
def parse_arguments() -> argparse.Namespace:
    # Parse command-line paths and experimental thresholds.
    parser = argparse.ArgumentParser(
        description=(
            "Generate, score and cluster probabilistic identity matches."
        )
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        "--automatic-threshold",
        type=float,
        default=AUTOMATIC_MATCH_THRESHOLD,
        help=(
            "Minimum weighted score for automatic matching. "
            f"Default: {AUTOMATIC_MATCH_THRESHOLD}"
        ),
    )
    parser.add_argument(
        "--review-threshold",
        ...
        help=(
            "Minimum weighted score for the review band. "
            f"Default: {REVIEW_MATCH_THRESHOLD}"
        ),
    )
    parser.add_argument(
        "--minimum-coverage",
        type=float,
        default=MINIMUM_AUTOMATIC_COVERAGE,
        help=(
            "Minimum available evidence weight for automatic matching. "
            f"Default: {MINIMUM_AUTOMATIC_COVERAGE}"
        ),
    )
    parser.add_argument(
        "--max-block-size",
        type=int,
        default=MAXIMUM_BLOCK_SIZE,
        help=(
            "Maximum clusters allowed in one blocking key. "
            f"Default: {MAXIMUM_BLOCK_SIZE}"
        ),
    )
...
def validate_parameters(arguments: argparse.Namespace) -> None:
    # Validate thresholds and the block-size safeguard.
    thresholds = {
        "automatic threshold": arguments.automatic_threshold,
        "review threshold": arguments.review_threshold,
        "minimum coverage": arguments.minimum_coverage,
    }
    for name, value in thresholds.items():
        if not 0.0 <= value <= 1.0:
            raise ValueError(f"The {name} must be between 0 and 1.")

    if arguments.review_threshold > arguments.automatic_threshold:
        raise ValueError(
            "The review threshold cannot exceed the automatic threshold."
        )
    if arguments.max_block_size < 2:
...
def classify_score(
    ...
    # Apply transparent experimental decision bands and evidence rules.
...
def build_summary(
    ...
    add_summary_section(rows, "EXPERIMENTAL PARAMETERS")
    rows.extend(
        [
            {
...
def main() -> None:
    ...
    results = run_probabilistic_matching(
        ...
        arguments.automatic_threshold,
        arguments.review_threshold,
        arguments.minimum_coverage,
        arguments.max_block_size,
    )

    print("Probabilistic identity resolution completed successfully.")
